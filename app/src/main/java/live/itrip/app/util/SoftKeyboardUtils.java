...
public class SoftKeyboardUtils {
    ...
    public static int getMaxPanelHeight(final Resources res) {
        if (MAX_PANEL_HEIGHT == 0) {
            MAX_PANEL_HEIGHT = (int) DeviceUtils.dipToPx(res, 300);
        }
    ...
    public static int getMinPanelHeight(final Resources res) {
        if (MIN_PANEL_HEIGHT == 0) {
            MIN_PANEL_HEIGHT = (int) DeviceUtils.dipToPx(res, 92);
        }
    ...
    public static int getMinKeyboardHeight(Context context) {
        if (MIN_KEYBOARD_HEIGHT == 0) {
            MIN_KEYBOARD_HEIGHT = (int) DeviceUtils.dipToPx(context.getResources(), 56);
        }
    ...
    public static ViewTreeObserver.OnGlobalLayoutListener attach(final Activity activity, IPanelHeightTarget target) {
        ...
        final boolean isFullScreen = UIUtils.isFullScreen(activity);
        final boolean isTranslucentStatus = UIUtils.isTranslucentStatus(activity);
        final boolean isFitSystemWindows = UIUtils.isFitsSystemWindows(activity);
    ...
    private static class KeyboardStatusListener implements ViewTreeObserver.OnGlobalLayoutListener {
        ...
        KeyboardStatusListener(boolean isFullScreen,
            ...
            this.statusBarHeight = UIUtils.getStatusBarHeight(contentView.getContext());
        ...
        private void calculateKeyboardHeight(final int displayHeight) {
            ...
            if (previousDisplayHeight == 0) {
                ...
                panelHeightTarget.refreshHeight(SoftKeyboardUtils.getValidPanelHeight(getContext()));
                return;
            }
            ...
            if (isHandleByPlaceholder(isFullScreen, isTranslucentStatus,
                ...
                AppLog.d(TAG, String.format("action bar over layout %s display height: %s",
            ...
            AppLog.d(TAG, String.format("pre display height: %s display height: %s keyboard: %s ",
            ...
            if (keyboardHeight == this.statusBarHeight) {
                AppLog.e(TAG, String.format("On global layout change get keyboard height just equal" +
                        " statusBar height %s", keyboardHeight));
                return;
            }

            // save the keyboardHeight
            boolean changed = SoftKeyboardUtils.saveKeyboardHeight(getContext(), keyboardHeight);
            if (changed) {
                final int validPanelHeight = SoftKeyboardUtils.getValidPanelHeight(getContext());
        ...
        private void calculateKeyboardShowing(final int displayHeight) {
            ...
            if (isHandleByPlaceholder(isFullScreen, isTranslucentStatus,
                ...
                if (!isTranslucentStatus &&
                    ...
                    AppLog.e(TAG, String.format("skip the keyboard status calculate, the current" +
            ...
            if (lastKeyboardShowing != isKeyboardShowing) {
                AppLog.d(TAG, String.format("displayHeight %s actionBarOverlayLayoutHeight %s " +
