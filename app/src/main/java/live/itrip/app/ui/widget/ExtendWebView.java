...
public class ExtendWebView extends WebView {
    ...
    public ExtendWebView(Context context, AttributeSet attrs, int defStyleAttr, boolean privateBrowsing) {
...
//                        ImageGalleryActivity.show(getContext(), bigImageUrl);
    ...
    public void loadDetailDataAsync(final String content) {
        ...
        if (context != null && context instanceof Activity) {
            ...
            AppLog.e(ExtendWebView.class.getName(), "loadDetailDataAsync error, the Context isn't ok");
        }
    }

    public void loadDetailDataAsync(final String content, Runnable finishCallback) {
        this.setWebViewClient(new WebClient(finishCallback));
        Context context = getContext();
        if (context != null && context instanceof Activity) {
            final Activity activity = (Activity) context;
            AppUtils.runOnThread(new Runnable() {
    ...
    private static class WebClient extends WebViewClient implements Runnable {
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            UIUtils.openExternalBrowser(view.getContext(), url);
            return true;
        }
